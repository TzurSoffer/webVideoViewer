    ...
    def subpage(self, name):
        def generate_frames():
            while True:
                ...
                if frame is None:
                    yield Response(b'<h1>No subpage found for {name}</h1>', mimetype='text/html').get_data()
                else:
                    html_content = f'<img src="data:image/jpeg;base64,{frame}" alt="Video Feed" />'
                    yield Response(html_content, mimetype='text/html').get_data()
