...
class VideoStream:
    ...
    def _createTemplate(self, subpage):
        with open(self.htmlTemplate, "r") as f:
            template = f.read()
        template = template.replace("{videoFeed}", subpage)
        return(template)

    def _createSubpage(self, name):
        videoFeedName = f"{name}FrameVideoFeed"
        # videoFeedName = "videoFeed"
        template = self._createTemplate(videoFeedName)
        print(template)

        @self.app.route(f"/{name}")
        def index():
            return(render_template_string(template, videoFeedUrl=f"/{videoFeedName}"))
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            videoStream.imshow("gray", cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY))
